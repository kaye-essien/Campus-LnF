...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['claim'])) {
    ...
    if ($_SESSION['user_id'] == $item['user_id']) {
        ...
        $chk = $conn->prepare("SELECT id FROM claims WHERE item_id = ? AND claimant_id = ?");
        ...
        if ($chk->get_result()->num_rows > 0) {
            ...
            $ins = $conn->prepare("INSERT INTO claims (item_id, claimant_id, message) VALUES (?, ?, ?)");
...
if (isLoggedIn() && ($_SESSION['user_id'] == $item['user_id'] || isAdmin())) {
    $cr = $conn->prepare("SELECT c.*, u.name AS claimant_name, u.email AS claimant_email FROM claims c JOIN users u ON c.claimant_id = u.id WHERE c.item_id = ? ORDER BY c.created_at DESC");
    $cr->bind_param("i", $id);
    $cr->execute();
...
// handle claim approval/rejection (owner only)
...
<div class="container" style="max-width:780px">
    ...
    <div style="background:var(--surface);border:1px solid var(--border);border-radius:var(--radius);overflow:hidden;margin-top:1.5rem">
        ...
        <?php if ($item['image_path'] && file_exists('../uploads/' . $item['image_path'])): ?>
            <img src="/uploads/<?= htmlspecialchars($item['image_path']) ?>" style="width:100%;max-height:380px;object-fit:cover" alt="">
        <?php else: ?>
        ...
        <div style="padding:2rem">
            ...
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:0.75rem;margin-bottom:1.5rem;font-size:0.9rem;color:var(--muted)">
                ...
                <div>📅 <strong style="color:var(--text)"><?= $item['date_reported'] ? date('M j, Y', strtotime($item['date_reported'])) : 'Not specified' ?></strong></div>
            ...
            <details style="margin-top:1rem">
                ...
                <form method="POST" style="margin-top:1rem;background:var(--bg);padding:1rem;border-radius:var(--radius);border:1px solid var(--border)">
                    <div class="form-group">
                        <label>Message to reporter <span style="color:var(--muted)">(describe how you know it's yours)</span></label>
                        <textarea name="message" placeholder="e.g. It's a blue bag with a red zipper, I left it near the second row..." required></textarea>
                    </div>
            ...
            <?php elseif (!isLoggedIn()): ?>
                <p style="margin-top:1rem;color:var(--muted)">
                    <a href="/pages/login.php">Login</a> to submit a claim for this item.
                </p>
    ...
    <div style="margin-top:2rem">
        ...
        <div style="background:var(--surface);border:1px solid var(--border);border-radius:var(--radius);padding:1.25rem;margin-bottom:1rem">
            <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:0.5rem">
                ...
                <span class="badge badge-<?= $claim['status'] === 'pending' ? 'open' : ($claim['status'] === 'approved' ? 'found' : 'lost') ?>">
